# Chap09_Formal_InForma(1)

# 분석 절차
# 1단계 : 토픽분석(단어의 빈도수) <-- ★
# 2단계 : 연관어 분석(관련 단어 분석) 
# 3단계 : 감성 분석(단어의 긍정/부정 분석) 


##################################################
# 1. 토픽분석(텍스트 마이닝) 
# - 시각화 : 단어 빈도수에 따른 워드 클라우드
###################################################

## 1. 패키지 설치와 준비 
# "KoNLP", "Sejong", 


# 1) KoNLP 설치 : 한글 자연어 처리 패키지
# -- Ko : 한글 , NLP : 자연어
install.packages("KoNLP") 
# Warning in install.packages :
#  package ‘KoNLP’ is not available (for R version 3.6.2)

# [해결방법]
# 현재 R 버전에서 제공하지 않는 패키지(KoNLP) 설치하는 방법

# [단계1] 이전버전에서 설치가능한 패키지 확인
# https://cran.rstudio.com/bin/windows/contrib/3.5/ 경로에서 
# 단축키를 Ctrl+F 누르고 KoNLP 를 입력하면 찾을 수 없다.
# 
# https://cran.rstudio.com/bin/windows/contrib/3.4/ 경로에서 
# 단축키를 Ctrl+F 누르고 KoNLP 를 입력하면 'KoNLP_0.80.1.zip' 찾을 수 있다.


# [단계2] 이전 R version 설치하기 
install.packages("https://cran.rstudio.com/bin/windows/contrib/3.4/KoNLP_0.80.1.zip",
                 repos = NULL)
# repos = NULL : 현재 쓰고 있는 3.6 버전에 설치하겠다는 옵션 



# 2) Sejong 설치 : KoNLP와 의존성 있는 현재 버전의 Sejong 설치 
# -- '세종'이라는 사전을 통해 명사만 추출
install.packages("Sejong")

# 3) wordcloud 설치    
# -- 단어 시각화 패키지
install.packages("wordcloud") 

# 4) tm 설치 
# -- txt 전처리, DTM, TDM
install.packages("tm") 


# 5) 패키지 로딩
library(KoNLP) 
# Checking user defined dictionary!
# 에러: package or namespace load failed for ‘KoNLP’ in loadNamespace(i, c(lib.loc, .libPaths()), versionCheck = vI[[i]]):
#   ‘hash’이라고 불리는 패키지가 없습니다
install.packages("hash")
install.packages("tau")
install.packages("RSQLite")
install.packages("devtools")

# > library(KoNLP) 
# Checking user defined dictionary! -- 로딩 성공?

library(tm) # 전처리 용도 
# 필요한 패키지를 로딩중입니다: NLP

library(wordcloud) 

###########################################################

## 2. facebook_bigdata.txt 가져오기
# (1) file(file.choose(), encoding="UTF-8") : 파일 객체 가져오기
facebook <- file(file.choose(), encoding="UTF-8")
# &&& 3


# (2) readLines() : 줄 단위 TEXT FILE 읽기 
facebook_data <- readLines(facebook) 

head(facebook_data, 2) # 앞부분 6줄 보기 - 줄 단위 문장 확인 
str(facebook_data) # chr [1:76]
class(facebook_data)

facebook_data[76]



## 3. 세종 사전에 신규 단어 추가 (ex)고유명사
# term : 추가단어,  tag='ncn' : 명사지시 코드
user_dic <- data.frame(term=c("R 프로그래밍","페이스북","김진성","소셜네트워크"), tag='ncn')

# buildDictionary() : Sejong 사전에 신규 단어 추가 --- KoNLP 제공 
buildDictionary(ext_dic='sejong', user_dic = user_dic)
# 결과 >> 370961 words dictionary was built.

# 사전에 등록 됐는지 확인 : extractNoun
extractNoun("나는 김진성이고, 대한민국 사람이다.")
# [1] "나"     "김진성" "대한"   "민국"   "사람"   => 단어 5개 원소

paste(extractNoun("나는 김진성이고, 대한민국 사람이다."), collapse=" ")
# "나 김진성 대한 민국 사람"

## 4. 단어추출 사용자 함수 정의
# (1) 사용자 정의 함수 실행 순서 
#    : 문장이 들어오면 문자형으로 형변환(as.character) 
#       >> 명사 추출(extractNoun) >> " "기준으로 명사형 이어짐(paste) 
exNouns <- function(x) { 
  paste(extractNoun(as.character(x)), collapse=" ")
}

# (2) exNouns 함수 이용 단어 추출 
# 형식) sapply(vector, 함수) -> 문장에서 단어 추출 
facebook_nouns <- sapply(facebook_data, exNouns) 


# (3) 단어 추출 결과
str(facebook_nouns) 
# Named chr [1:76]

facebook_nouns[1] 
# &&& 4
facebook_nouns[2]


## 5. 데이터 전처리   
# tm 패키지에서 제공하는 함수 : Corpus, VectorSource, inspect

# (1) 말뭉치(코퍼스:Corpus) 생성 : 텍스트를 처리할 수 있는 자료의 집합 
# 벡터 소스 생성 -> 코퍼스 생성  / # Corpus : 문서의 단어를 행열구조로 
myCorpus <- Corpus(VectorSource(facebook_nouns))  


myCorpus
# 결과) documents: 76 --- 76개 원소


inspect(myCorpus[1]) # corpus 내용 보기 
inspect(myCorpus[2])

# (2) 데이터 전처리 : 말뭉치 대상 전처리 
# tm_map(x, func) 
# func : removePunctuation, removeNumbers, tolower, removeWords, stopwords()
myCorpusPrepro <- tm_map(myCorpus, removePunctuation) # 문장부호 제거-removePunctuation
myCorpusPrepro <- tm_map(myCorpusPrepro, removeNumbers) # 수치 제거-removeNumbers
myCorpusPrepro <- tm_map(myCorpusPrepro, tolower) # 소문자 변경
myCorpusPrepro <-tm_map(myCorpusPrepro, removeWords, stopwords('english')) # 불용어제거

stopwords('english') # 불용어 : "i", "me", "my" ~ "too", "very" 총 174개

inspect(myCorpusPrepro[1])
# &&&5

# (3) 전처리 결과 확인 
myCorpusPrepro # Content:  documents: 76
inspect(myCorpusPrepro[1:2]) # 데이터 전처리 결과 확인(숫자, 영문 확인)


## 6. 단어 선별(단어 길이 2개 이상)

# (1) 단어길이 2개 이상(한글 1개 2byte) 단어 선별 -> matrix 변경
# wordLengths=c(4,16) : 2 ~ 8음절 => 1개 단어, 9개 이상 단어 제외
myCorpusPrepro_term <- TermDocumentMatrix(myCorpusPrepro, 
                                          control=list(wordLengths=c(4,16))) 
# TermDocumentMatrix : [행:Term(단어), 열:Doc(문장)]

myCorpusPrepro_term
# <<TermDocumentMatrix (terms: 696, documents: 76)>> -- 696 x 76 =  52,896(셀 개수)
#   Non-/sparse entries: 1256/51640
# Sparsity           : 98% -- 희소비율(1의 비율)
# Maximal term length: 12 -- 6음절 (6개 음절을 가진 단어가 가장 긴 단어)
# Weighting          : term frequency (tf)


# -참고-
inspect(myCorpusPrepro_term[1:2, 1:2])


# (2) Corpus -> 평서문 변환 : matrix -> data.frame 변경
myTerm_df <- as.data.frame(as.matrix(myCorpusPrepro_term)) 
str(myTerm_df)
#'data.frame':	696 obs. of  76 variables

myTerm_df[c(1:10), 1]
# [1] 1 1 1 1 2 1 1 1 3 1 -- 해당 단어의 출연 빈도수


## 7. 단어 빈도수 구하기
# (1) 단어 빈도수 내림차순 정렬
wordResult <- sort(rowSums(myTerm_df), decreasing=TRUE) 
wordResult[1:5]
# 데이터     분석 빅데이터     처리     사용 -- 칼럼이름 
#    91       41       33       31       29  -- vector data

w_name <- names(wordResult)
w_name[1:5]
# "데이터" "분석" "빅데이터" "처리" "사용"  


#########################################################################
# (2) 불용어 제거 : 의미없는 단어 제거 (ex) '사용' 단어 제거하기
# - 위의 단계 다시 반복 - 
# [단계1] 데이터 전처리 : tm_map(myCorpusPrepro, removeWords, c(stopwords('english'), '사용')
myCorpusPrepro <- tm_map(myCorpus, removePunctuation) 
myCorpusPrepro <- tm_map(myCorpusPrepro, removeNumbers) 
myCorpusPrepro <- tm_map(myCorpusPrepro, tolower) 
myCorpusPrepro <- tm_map(myCorpusPrepro, removeWords, c(stopwords('english'), '사용'))

# [단계2] 단어문서 행렬 생성
myCorpusPrepro_term <- TermDocumentMatrix(myCorpusPrepro, 
                                          control=list(wordLengths=c(4,16))) 

# [단계3] 말뭉치 -> 평서문
myTerm_df <- as.data.frame(as.matrix(myCorpusPrepro_term)) 

# [단계4] 내림정렬
wordResult <- sort(rowSums(myTerm_df), decreasing=TRUE)
wordResult[1:5]
# 데이터     분석 빅데이터     처리     수집 
#    91       41       33       31       27 

############################################################################

## 8. 단어구름에 디자인 적용(빈도수, 색상, 랜덤, 회전 등)
# (1) 단어 이름 생성 -> 빈도수의 이름
myName <- names(wordResult)  

# (2) 단어이름과 빈도수로 data.frame 생성
word.df <- data.frame(word=myName, freq=wordResult) 
head(word.df)
str(word.df) # word, freq 변수

# (3) 단어 색상과 글꼴 지정
pal <- brewer.pal(12,"Paired") # 12가지 색상 pal <- brewer.pal(9,"Set1") # Set1~ Set3
# 폰트 설정세팅 : "맑은 고딕", "서울남산체 B"
windowsFonts(malgun=windowsFont("맑은 고딕"))  #windows

# (4) 단어 구름 시각화 - 별도의 창에 색상, 빈도수, 글꼴(family), 회전 등의 속성을 적용하여 
wordcloud(word.df$word, word.df$freq, 
          scale=c(5,1), min.freq=2, random.order=F, 
          rot.per=.1, colors=pal, family="malgun")

# min.freq=2 : 최소 출연 빈도수
# random.order=F : 회전, rot.per=.1 : 각도




