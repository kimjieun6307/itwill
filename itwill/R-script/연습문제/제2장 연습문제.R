...
data.frame(kor, eng, mat)


# 조건2) 행/열 방향으로 max()함수를 적용하여 최댓값 구하기
apply(data.frame(kor, eng, mat), 1, max)
apply(data.frame(kor, eng, mat), 2, max)

# 조건3) 행/열 방향으로 mean()함수를 적용하여 평균 구하기(소숫점 2자리 까지 표현)
#  힌트 : round(data, 자릿수)
round(apply(data.frame(kor, eng, mat), 1, mean), 2)
round(apply(data.frame(kor, eng, mat), 2, mean), 2)

# 조건4) 행 단위 분산과 표준편차 구하기  
#  힌트 : var(), sd()
apply(data.frame(kor, eng, mat), 1, var)
apply(data.frame(kor, eng, mat), 2, var)
apply(data.frame(kor, eng, mat), 1, sd)
apply(data.frame(kor, eng, mat), 2, sd)
...
str_extract_all(Data2, "[0-9]{4}[가-힣]")
