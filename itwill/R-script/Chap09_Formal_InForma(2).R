...
close(marketing)  # marketing 객체 메모리에서 지움
...
# 2. 줄 단위 단어 추출
#----------------------------------------------------
# Map(extractNoun, )함수 이용 줄 단위 단어 추출 
...
# is.hangul() : KoNLP 패키지 제공, 한글만 필터링 해주는 역할
# Filter(f, x) : base
# nchar() : base -> 문자 수 반환
...
# >> "트랜잭션" 객체로 만든후 작업해야함. as()사용 / 트랜잭션 : 거래 정보
#----------------------------------------------------
# arules 패키지 설치
...
# 353 transactions (rows) and >> doc number
# 2423 items (columns  >> word number
...
# 형식) apriori(트랜잭션 객체data, parameter = NULL, appearance = NULL, control = NULL)
